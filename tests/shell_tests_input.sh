...
check()
{
    ...
    if [ "${1}" == "from_file" ]; then
        ...
        echo "${1}" | ../builddir/42sh > "actual result"
    fi
    ...
    if [ "${1}" == "from_file" ]; then
        ...
        echo "${1}" | dash > "expected result"
        expected=$?
    else
        #bash -c "${1}" > "expected result"
        echo "${1}" | bash > "expected result"
        expected=$?
    fi
    # EXECUTION ##############################################
    ...
    if [ "${1}" == "from_file" ]; then
        ...
        echo -e -n "\033[1m\033[4mTest $test_count :\033[0m echo \"${1}\" | ./builddir/42sh "
    fi
...
path="$(find ~+ -type f -name 'shell_tests_input.sh')"
path="${path::-20}"
cd "$path"
...
echo -e "\e[93mUse \e[1m'./shell_tests_input.sh print'\e[21m to display successful tests too.\e[0m"
echo -e "\e[93mUse \e[1m'./shell_tests_input.sh no_error'\e[21m to hide stderr and leaks.\e[0m\n"
